      ...
      <v-col cols="10">
        <v-data-table :headers="headers" :items="userList" :items-per-page="5" class="elevation-1">
          <template v-slot:[`item.actions`]="{ item }">
            ...
            <v-btn class="mr-2" color="primary" fab x-small elevation="2" @click="onClickUpdateUserStatus(item)">
              <v-icon>edit</v-icon>
            </v-btn>
          </template>
        </v-data-table>
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          <v-form ref="updateForm" v-model="updateForm">
            <!-- User ID -->
            <v-text-field v-model="toUpdateUser.id" label="User ID" disabled></v-text-field>

            <!-- User Name -->
            <v-text-field v-model="toUpdateUser.name" label="User Name" disabled></v-text-field>

            <!-- User Gmail -->
            <v-text-field v-model="toUpdateUser.gmail" label="User Gmail" disabled></v-text-field>

            <!-- User Role -->
            <v-text-field v-model="toUpdateUser.role" label="User Role" disabled></v-text-field>

            <!-- User Start Join Date -->
            <v-text-field v-model="toUpdateUser.startJoinDate" label="User Start Join Date" disabled></v-text-field>

            <!-- User Last Join Date -->
            <v-text-field v-model="toUpdateUser.lastJoinDate" label="User Last Join Date" disabled></v-text-field>


            <!-- User Status -->
            <v-select v-model="userStatus" :items="userStatusList" :rules="[(v) => !!v || 'Required']" label="User Status"
              required>
            </v-select>
        ...
        <v-card-actions class="justify-end">
          <v-btn :disabled="!updateForm" color="success" @click="updateUserStatus">
